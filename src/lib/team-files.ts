...
export function buildExportScopeOptions({ projectFilter, bots, groups }: ExportScopeContext): ExportScopeOption[] {
  ...
  for (const section of sections) {
    ...
    const groupIds = groups
      .filter((group) => !group.dm && group.memberIds.filter((id) => activeIds.has(id)).every((id) => projectIdSet.has(id)) && (group.section?.trim() === section || (!group.section && group.memberIds.some((id) => projectIdSet.has(id)))))
      .map((group) => group.id)
