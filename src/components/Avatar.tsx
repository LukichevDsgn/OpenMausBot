...
export type MausAvatarProps = { color: MausColor; avatarDefinition?: BotProceduralAvatar | null; bodyId?: MascotBodyId | null; state?: MausState; expression?: number; size?: number; label?: string; motion?: MausMotion; motionKey?: number; turn?: number; gaze?: { x?: number; y?: number }; spring?: number; eyeScale?: number; showMouth?: boolean; mouthStroke?: number; forward?: boolean; lookAround?: number; trackPointer?: boolean; animated?: boolean; autoBlink?: boolean; autoExpression?: boolean; eyeSpacing?: number; faceX?: number; faceY?: number; faceScale?: number };
...
function MausAvatarComponent({ color, avatarDefinition, bodyId, state = "idle", expression, size = 44, label, motion = "none", motionKey = 0, turn, gaze, spring, eyeScale, showMouth = true, mouthStroke = MOUTH_STROKE, forward = true, lookAround, trackPointer = true, animated = true, autoBlink = animated, autoExpression = animated }: MausAvatarProps, ref: React.Ref<MausAvatarHandle>) {
  ...
  const renderedSilhouette = bodyId && !avatarDefinition
    ? MASCOT_BODIES[botMascotBody(bodyId)]
    : silhouetteFor(definition);
...
export type BotAvatarProps = Omit<MausAvatarProps, "color"> & { bot: { name?: string; color: MausColor; avatarUrl?: string | null; avatarCrop?: BotAvatarCrop; avatarDefinition?: BotProceduralAvatar | null; mascotBody?: MascotBodyId | null }; onImageError?: () => void };
export function BotAvatar({ bot, size = 44, label, onImageError, ...props }: BotAvatarProps) {
  ...
  if (profile.avatarCrop === "mascot" || !profile.avatarUrl || failed) return <MausAvatar bodyId={bot.mascotBody ?? undefined} {...props} color={bot.color} avatarDefinition={profile.avatarDefinition} size={size} label={label ?? bot.name} />;
