...
type Draft = Omit<Endpoint, "configured" | "context"> & {
  configured?: boolean;
  context: string;
  clearKey?: boolean;
...
function toPayload(draft: Draft) {
  return {
    ...draft,
    id: draft.id || draft.providerId,
    configured: undefined,
    context: draft.context.trim() ? Number(draft.context) : undefined,
    [API_KEY_FIELD]: draft.apiKey.trim() || undefined,
...
export function CustomEndpointsPanel() {
  ...
  const editing = useMemo(() => endpoints.some((endpoint) => endpoint.id === draft?.id), [draft?.id, endpoints]);
  const set = <K extends keyof Draft>(key: K, value: Draft[K]) => {
    setDraft((current) => current
      ? { ...current, [key]: value, clearKey: key === "apiKey" ? false : current.clearKey }
      : current);
    setProbe(null);
  };
  ...
  const save = async () => {
    ...
    try {
      ...
      const result = window.ogb?.saveCustomEndpoint
        ...
        : await api(`/api/custom-endpoints/${encodeURIComponent(payload.id)}`, { method: "PUT", body: JSON.stringify(payload) });
              ...
              {open && (
                <div className="mt-2 border-t border-hairline/30 pt-2 text-[11px] text-ink-secondary">
                  ...
                  <div className="mt-2 flex justify-end gap-2">
                    <button type="button" onClick={() => openNew({ ...endpoint, context: endpoint.context ? String(endpoint.context) : "", [API_KEY_FIELD]: "" })} className="flex items-center gap-1 rounded px-2 py-1 text-ink-secondary hover:bg-raised hover:text-ink"><Pencil size={12} /> Edit</button>
      ...
      {draft && (
        <div className="mt-3 border-t border-hairline/30 pt-3">
          ...
          {draft.configured && (
            <button
              type="button"
              onClick={() => setDraft((current) => current ? { ...current, [API_KEY_FIELD]: "", clearKey: !current.clearKey } : current)}
              className={cn("mt-1.5 text-[11px]", draft.clearKey ? "text-danger" : "text-ink-secondary hover:text-ink")}
            >
              {draft.clearKey ? "Saved key will be removed" : "Remove saved key"}
            </button>
          )}
