 ...
 * it. Each `viewBox` is the box that actually contains its outline, which is the
 * stage for all of them but the blob. The `cursor` outline is the one exception
 * to all of this: it is shipped artwork, copied verbatim.
 */
...
export interface ShapeDef {
  ...
  d: string
  /** `[minX, minY, width, height]` of the space `d` is drawn in. */
 ...
 *
 * Drawn pre-translate: the component paints it under `translate(210, 80)` inside a
 * 512x512 face box, so the raw coordinates below run negative. The viewBox given
 * below is the 512x512 space the translate lands it in, not the extent of `d`.
 */
