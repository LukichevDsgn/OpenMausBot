...
interface QueueEntry {
  ...
  botId: string;
  items: Array<{ messageId: string; text: string; prompt: string; replyToId?: string; skillIds: string[]; legacySkillId?: string }>;
}
...
export function queueSteeredMessage(
  bot: BotRecord,
  text: string,
  options: { prompt?: string; replyToId?: string; skillIds?: readonly string[]; skillId?: string } = {},
): { id: string } {
  ...
  entry.items.push({
    ...
    skillIds: [...new Set(options.skillIds ?? (options.skillId ? [options.skillId] : []))],
    legacySkillId: options.skillIds === undefined ? options.skillId : undefined,
  });
...
export function drainSteeredMessages(
  ...
  for (const [threadId, entry] of [...queues]) {
    ...
    const firstSkillIds = entry.items[0]?.skillIds ?? [];
    const sameSelection = (left: readonly string[], right: readonly string[]) =>
      left.length === right.length && left.every((id, index) => id === right[index]);
    const boundary = entry.items.findIndex((item) => !sameSelection(item.skillIds, firstSkillIds));
    ...
    void run(
      ...
      entry.items[0]?.legacySkillId ?? (firstSkillIds.length ? [...firstSkillIds] : undefined),
    );
  }
}
