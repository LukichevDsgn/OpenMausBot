...
describe("steer-queue e2e (fake ACP fleet)", () => {
  ...
  it(
    ...
    async () => {
      ...
      const selectedTask = snapshot.tasks.find((task: any) => task.threadId === snapshot.threadId);
      expect(selectedTask.lifecycle).toMatchObject({
        state: "completed",
        terminalReceipt: { state: "completed", eventId: expect.any(String) },
      });
    },
    60_000,
  );
