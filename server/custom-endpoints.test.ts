...
const endpoint = parseCustomEndpoint({
  ...
  context: 1_000_000,
  discoverModels: true,
  apiKey: "secret-value", // secret-scan: allow-test-fixture
});
...
function makeEndpoint(id: string, overrides: Partial<typeof endpoint> = {}) {
  return parseCustomEndpoint({
    ...
    discoverModels: true,
    apiKey: `key-${id}`, // secret-scan: allow-test-fixture
    ...overrides,
  });
}
...
describe("custom OpenAI-compatible endpoints", () => {
  ...
  it("maps dashed endpoint ids to portable environment names", () => {
    expect(customEndpointKeyEnv("local-proxy")).toBe("OPENMAUSBOT_ENDPOINT_LOCAL_DPROXY_API_KEY");
    expect(customEndpointKeyEnv("local_proxy")).toBe("OPENMAUSBOT_ENDPOINT_LOCAL_UPROXY_API_KEY");
  });
  ...
  it("sanitizes forged legacy URLs for public, probe, and child overlay paths", async () => {
    const unsafeBaseUrl = "https://legacy-user:legacy-pass@example.test/api?token=legacy-secret#fragment"; // secret-scan: allow-test-fixture
  ...
  it("discovers models with the endpoint key and keeps the default bounded", async () => {
    ...
    const catalog = await fetchCustomEndpointModels(
      ...
      async (input, init) => {
        ...
        calls.push({ url: String(input), authorization: String(headers?.authorization) });
  ...
  it("uses a child-local overlay and leaves user OpenCode config byte-identical", async () => {
    ...
    try {
      ...
      expect(config.provider.openrouter.options).toMatchObject({
        baseURL: "https://openrouter.ai/api/v1",
        apiKey: "{env:OPENMAUSBOT_ENDPOINT_OPENROUTER_API_KEY}", // secret-scan: allow-test-fixture
      });
  ...
  it("keeps concurrent child overlays independent", async () => {
    ...
    await Promise.all([
      Promise.resolve(ensureOpenCodeCustomEndpointModel(
        "endpoint::openrouter::z-ai%2Fglm-5.2",
        [{ ...endpoint, apiKey: "secret-a" }], // secret-scan: allow-test-fixture
        envA,
      )),
      Promise.resolve(ensureOpenCodeCustomEndpointModel(
        "endpoint::openrouter::qwen%2Fqwen3-coder",
        [{ ...endpoint, apiKey: "secret-b", defaultModel: "qwen/qwen3-coder" }], // secret-scan: allow-test-fixture
        envB,
      )),
    ]);
