...
async function route(
  ...
  if (request.method === "GET" && url.pathname === "/v1/bot-shares") {
    return listBotShares(request, env, auth, baseURL);
  }
  if (request.method === "POST" && url.pathname === "/v1/bot-shares") {
    return createBotShare(request, env, auth, baseURL);
  }
  const shareVersion = url.pathname.match(BOT_SHARE_VERSION_ROUTE);
  if (request.method === "POST" && shareVersion) {
    return updateBotShare(request, shareVersion[1], env, auth, baseURL);
  }
  const shareVisibility = url.pathname.match(BOT_SHARE_VISIBILITY_ROUTE);
  if (request.method === "POST" && shareVisibility) {
    return updateBotShareVisibility(request, shareVisibility[1], env, auth, baseURL);
  }
