...
// JSON framing for a maximum-size Markdown string stays bounded as well. The
// package itself is checked independently after UTF-8 decoding.
export const BOT_SHARE_PUBLISH_BODY_MAX_BYTES = BOT_SHARE_PACKAGE_MAX_BYTES + 16 * 1024;

const COLORS = ["green", "blue", "red", "orange", "purple", "cyan", "pink", "yellow", "teal", "coral"] as const;
const SILHOUETTES = ["cursor", "orb", "tile", "gem", "pebble", "spark", "capsule", "shield", "leaf", "moon", "hex", "drop"] as const;
const EYES = ["balanced", "wide", "calm"] as const;
const MOUTHS = ["soft", "bold", "none"] as const;
const EXPRESSION_PRESETS = ["steady", "bright", "grin", "soft-focus", "wry", "side-eye", "open", "downcast", "serious", "warm", "alert", "delighted", "pondering", "listening", "skeptical", "surprised", "playful", "relieved", "confident", "resting"] as const;
const AVATAR_PRESETS = ["strobi", "freddy", "citrus", "nova", "grok-bot", "sunee", "kirby", "cloudee", "cubee", "onee"] as const;
const RESTING_ANIMATIONS = ["sleeping", "waking", "idle", "listening", "thinking", "searching", "working", "excited", "bored", "suspicious", "angry", "drowsy", "happy", "curious", "confused", "surprised", "proud", "shy", "sad", "laughing", "scared", "playful", "celebrate"] as const;
...
const avatarSchema = z.strictObject({
  version: z.literal(1),
  seed: requiredText(80).regex(/^[A-Za-z0-9_-]+$/),
  silhouette: z.enum(SILHOUETTES),
  eyeStyle: z.enum(EYES),
  mouthStyle: z.enum(MOUTHS),
  expressionPreset: z.enum(EXPRESSION_PRESETS).optional(),
  avatarPresetId: z.enum(AVATAR_PRESETS).optional(),
  restingAnimationId: z.enum(RESTING_ANIMATIONS).optional(),
});
const packageDefinitionSchema = z.object({
  ...
  agents: z.array(z.object({
    ...
    appearance: z.object({
      ...
      avatarDefinition: avatarSchema.optional(),
    }),
...
const updateSchema = z.strictObject({
  packageMarkdown: z.string().min(1),
  expectedActiveVersion: z.number().int().min(1),
});
...
interface PublicShareRow extends ShareRow {
...
async function packageSnapshot(markdown: string) {
  const inputBytes = new TextEncoder().encode(markdown).byteLength;
  if (inputBytes > BOT_SHARE_PACKAGE_MAX_BYTES) throw new HTTPError(413, "package_too_large");
  let parsed: PackageDefinition;
  try {
    parsed = parsePackageMarkdown(markdown);
  } catch {
    throw new HTTPError(400, "invalid_package");
  }
  // Store the canonical renderer output, not the caller's raw Markdown.
  // Unknown YAML/body additions are therefore unable to smuggle credentials,
  // grants, paths, transcripts, memory, provider/model, or runtime state.
...
function shareJSON(row: ShareRow, baseURL: string) {
  return {
    ...
    byteSize: row.package_bytes,
    createdAt: row.created_at,
    updatedAt: row.updated_at,
    versionCreatedAt: row.version_created_at,
    shareUrl: `${baseURL}/s/${row.id}`,
...
const activeShareSelect = `
  SELECT s.id, s.visibility, s.active_version, s.created_at, s.updated_at,
    ...
    FROM bot_shares s
    JOIN bot_share_versions v
      ON v.share_id = s.id AND v.version = s.active_version`;
...
async function ownedShare(shareId: string, ownerUserId: string, env: Env): Promise<ShareRow | null> {
  ...
  return env.DB.prepare(
    `${activeShareSelect}
      WHERE s.id = ? AND s.owner_user_id = ? AND s.deleted_at IS NULL`,
  ).bind(shareId, ownerUserId).first<ShareRow>();
}

export async function listBotShares(
  request: Request,
  env: Env,
...
export async function createBotShare(
  request: Request,
  env: Env,
  auth: ControlPlaneAuth,
  baseURL: string,
  ...
  try {
    await env.DB.batch([
      env.DB.prepare(
        `INSERT INTO bot_shares
          (id, owner_user_id, visibility, active_version, created_at, updated_at)
         VALUES (?, ?, ?, 1, ?, ?)`,
      ).bind(shareId, session.user.id, parsed.data.visibility ?? "unlisted", now, now),
      env.DB.prepare(
        `INSERT INTO bot_share_versions
          (share_id, version, name, summary, package_markdown, package_sha256, package_bytes, created_at)
         VALUES (?, 1, ?, ?, ?, ?, ?, ?)`,
      ).bind(shareId, snapshot.name, snapshot.summary, snapshot.markdown, snapshot.sha256, snapshot.bytes, now),
    ]);
  } catch (error) {
    if (error instanceof Error && /UNIQUE constraint failed/i.test(error.message)) {
      throw new HTTPError(409, "share_id_conflict");
    }
    throw error;
  }
...
export async function updateBotShare(
  request: Request,
  shareId: string,
  env: Env,
  auth: ControlPlaneAuth,
  ...
  if (current.active_version !== parsed.data.expectedActiveVersion) {
  ...
  try {
    await env.DB.prepare(
      ...
      shareId,
      parsed.data.expectedActiveVersion + 1,
      snapshot.name,
      snapshot.summary,
      snapshot.markdown,
      snapshot.sha256,
    ...
    if (error instanceof Error && /bot_share_version_conflict|UNIQUE constraint failed/i.test(error.message)) {
      throw new HTTPError(409, "version_conflict");
    }
    throw error;
  }
...
export async function updateBotShareVisibility(
  request: Request,
  shareId: string,
  env: Env,
  auth: ControlPlaneAuth,
  ...
  const result = await env.DB.prepare(
    `UPDATE bot_shares SET visibility = ?, updated_at = ?
...
export async function deleteBotShare(
  request: Request,
  shareId: string,
  env: Env,
  auth: ControlPlaneAuth,
  ...
  const now = Date.now();
  const result = await env.DB.prepare(
    `UPDATE bot_shares SET deleted_at = ?, updated_at = ?
      WHERE id = ? AND owner_user_id = ? AND deleted_at IS NULL`,
...
async function publicShare(shareId: string, env: Env): Promise<PublicShareRow | null> {
  ...
  return env.DB.prepare(
    `SELECT s.id, s.visibility, s.active_version, s.created_at, s.updated_at,
            v.name, v.summary, v.package_sha256, v.package_bytes,
            v.created_at AS version_created_at, v.package_markdown
       FROM bot_shares s
       JOIN bot_share_versions v
         ON v.share_id = s.id AND v.version = s.active_version
      WHERE s.id = ? AND s.visibility = 'unlisted' AND s.deleted_at IS NULL`,
...
export async function publicBotSharePackage(shareId: string, env: Env): Promise<Response> {
  ...
  return new Response(row.package_markdown, {
    headers: {
...
export async function publicBotShareLanding(shareId: string, env: Env, baseURL: string): Promise<Response> {
  ...
  const installURL = `openmausbot://install?url=${encodeURIComponent(packageURL)}`;
  const html = `<!doctype html><html lang="en"><head><meta charset="utf-8"><meta name="viewport" content="width=device-width,initial-scale=1"><title>${escapeHTML(row.name)} · OpenMausBot</title></head><body><main><h1>${escapeHTML(row.name)}</h1><p>${escapeHTML(row.summary)}</p><p>Version ${row.active_version}</p><p><a href="${escapeHTML(installURL)}">Open in OpenMausBot</a></p></main></body></html>`;
